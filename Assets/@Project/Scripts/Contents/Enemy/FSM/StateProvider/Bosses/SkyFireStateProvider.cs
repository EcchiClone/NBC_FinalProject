...
public class SkyFireStateProvider : BaseStateProvider<Boss_States>
{
    private Dictionary<Boss_States, BaseState<Boss_States>> _states = new Dictionary<Boss_States, BaseState>();
    // TODO : 추상화 상속 할거면 이것도 그냥 추상 클래스 쪽으로 올려도 될듯

    public SkyFireStateProvider(BaseStateMachine context)
        : base(context)
    {
        AddState(Boss_States.Alive, new SkyFire_AliveState(context, this));
